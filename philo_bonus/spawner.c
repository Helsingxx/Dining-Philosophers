...
/*   Updated: 2023/12/21 18:02:43 by eamrati          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void wait_for_childs(int *launched_child_pids, int nbr_philos)
{
	...
	return_pid = waitpid(-1, &exit_status, WEXITED);
	while (return_pid != -1)
	{
		if (exit_status == PHIL_DIED)
		{
...
void fork_handler(t_arg* arguments_from_argv, pid_t chld_pid, int ecx, int *launched_child_pids)
{
	...
	if (!chld_pid)
	{
		if (launch_philosopher_and_monitor(arguments_from_argv) == PHIL_THREAD_FAIL_CREATE)
